    ...
    <div className="mx-auto max-w-md">
        ...
        <div className="relative flex rounded-xl bg-surface-2 p-1 ring-1 ring-white/5">
          {['search', 'ai'].map((m) => (
            ...
            >
              ...
              <span className={mode === m ? 'text-surface' : 'text-slate-400'}>
                {m === 'ai' ? '✨ AI' : 'Search'}
              </span>
            </button>
          ))}
        ...
        {mode === 'search' ? (
          <motion.div key="search" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            {recents.length > 0 && (
              <section className="mt-4 px-5">
                <p className="mb-2 text-xs font-medium uppercase tracking-wide text-slate-500">Quick add</p>
            ...
            <section className="mt-4 px-5">
              ...
              {!loading && error && (
                <p className="rounded-xl bg-pink-500/10 p-4 text-center text-sm text-pink-300">{error}</p>
              )}
          ...
          >
            ...
            <textarea
              ...
              className="w-full resize-none rounded-xl bg-surface-2 px-4 py-3 text-base text-white placeholder:text-slate-500 outline-none ring-1 ring-white/10 focus:ring-brand"
            />
            <button
            ...
            {aiError && <p className="mt-3 text-center text-sm text-pink-300">{aiError}</p>}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <section className="mt-5 px-5">
        <button
          ...
          className="w-full rounded-xl bg-surface-2 px-4 py-3 text-sm font-semibold text-slate-200 ring-1 ring-white/10 active:bg-white/5"
        >
          ✏️ Add manually
        </button>
      </section>
...
function PhotoButton({ icon, label, onClick, disabled }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className="flex flex-col items-center gap-1 rounded-2xl bg-surface-2 py-5 text-sm font-semibold text-white ring-1 ring-white/10 transition-colors active:bg-white/5 disabled:opacity-50"
    >
